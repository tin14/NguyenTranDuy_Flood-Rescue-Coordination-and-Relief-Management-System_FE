  ...
  <div>
    ...
    <div class="container py-4">
      ...
      <div v-if="activeTab === 'distribution'">
        <div class="d-flex justify-content-between align-items-center mb-3">
          <h5 class="fw-semibold mb-0"><i class="bi bi-truck text-success me-2"></i>Theo Dõi Phân Phối Hàng Cứu Trợ</h5>
        </div>
        ...
        <div class="table-responsive">
          <table class="table table-hover shadow-sm bg-white rounded">
            <thead class="table-light">
              <tr>
                <th>#</th><th>Hàng cứu trợ</th><th>Số lượng</th><th>Đội nhận</th><th>Phương tiện</th><th>Địa điểm phát</th><th>Ngày</th><th>Ghi chú</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr v-if="store.distributions.length === 0">
                <td colspan="8" class="text-center text-muted py-3">Chưa có lịch sử phân phối</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div v-if="activeTab === 'stats'">
        ...
        <!-- Bảng tình trạng phương tiện các đội cứu hộ -->
        <div class="card border-0 shadow-sm">
          <div class="card-header fw-semibold bg-white"><i class="bi bi-truck text-info me-2"></i>Tình Trạng Phương Tiện Các Đội Cứu Hộ</div>
          <div class="card-body p-0">
            <table class="table table-hover mb-0">
              <thead class="table-light">
                <tr><th>Đội cứu hộ</th><th>Phương tiện</th><th>Trạng thái đội</th><th>Nhiệm vụ hiện tại</th></tr>
              </thead>
              <tbody>
                <tr v-for="team in store.teams" :key="team.id">
                  <td class="fw-semibold">{{ team.name }}</td>
                  <td>{{ team.vehicles.join(', ') }}</td>
                  <td>
                    <span class="badge" :class="team.status === 'available' ? 'bg-success' : 'bg-warning text-dark'">
                      {{ team.status === 'available' ? '✅ Sẵn sàng' : '🔄 Đang bận' }}
                    </span>
                  </td>
                  <td class="text-muted small">{{ team.currentTask ? 'Yêu cầu #' + team.currentTask : '—' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
