          ...
          <div v-else class="card border-0 shadow-sm">
            ...
            <div class="card-body">
              ...
              <div class="border-top pt-3 mt-2">
                ...
                <!-- [3] PHÂN PHỐI ĐỒ TIẾP TẾ — chỉ hiện khi đã phân công đội (assigned/in_progress) -->
                <div v-if="['assigned', 'in_progress'].includes(selectedRequest.status)" class="mb-3">
                  <label class="form-label small fw-semibold"><i class="bi bi-box-arrow-up text-success me-1"></i>Phân phối đồ tiếp tế cho đội</label>
                  <!-- Chọn mặt hàng từ kho -->
                  <select v-model.number="distForm.itemId" class="form-select form-select-sm mb-2">
                  ...
                  <!-- Số lượng -->
                  <input v-model.number="distForm.quantity" type="number" class="form-control form-control-sm mb-2" min="1" placeholder="Số lượng" />
                  <!-- Chọn phương tiện vận chuyển của đội được phân công -->
                  <select v-model="distForm.vehicle" class="form-select form-select-sm mb-2">
                    <option value="">-- Chọn phương tiện vận chuyển --</option>
                    <option v-for="v in assignedTeamVehicles" :key="v" :value="v">{{ v }}</option>
                  </select>
                  <input v-model="distForm.note" class="form-control form-control-sm mb-2" placeholder="Ghi chú..." />
                  <div v-if="distError" class="alert alert-danger py-1 small mb-2">{{ distError }}</div>
                  <button class="btn btn-success btn-sm w-100" @click="doDistribute">
                    <i class="bi bi-box-arrow-up me-1"></i>Phân phối hàng
                  </button>
                </div>

                <!-- [4] HỦY YÊU CẦU — hiện với mọi trạng thái trừ completed và cancelled -->
...
const assignedTeamVehicles = computed(() => {
  if (!selectedRequest.value?.assignedTeamId) return []
  const team = store.teams.find(t => t.id === selectedRequest.value.assignedTeamId)
...
function selectRequest(req) {
  ...
  Object.assign(distForm, { itemId: '', quantity: 1, vehicle: '', note: '' }) // Reset distribution form
  distError.value = ''
}
...
// Phân phối đồ tiếp tế cho đội cứu hộ đang được phân công
function doDistribute() {
  distError.value = ''
  if (!distForm.itemId || !distForm.vehicle) { distError.value = 'Vui lòng chọn hàng hóa và phương tiện!'; return }
  const item = store.inventory.find(i => i.id === distForm.itemId)
  if (!item) return
